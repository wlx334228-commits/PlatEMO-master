classdef BLCMAESLSTR_NoLSTR < ALGORITHM
    ...
    % PlatEMO implementation of BL-CMA-ES without LSTR.
    methods
        function main(Algorithm,Problem)
            %% Parameters
            [uTol,lTol,~,~,includeLLConInUpper] = ...
                Algorithm.ParameterSet(1e-6,1e-6,350,25,0);
            ...
            function writeRunRecord()
                if ~runRecordWritten
                    AppendBLCMAESLSTRNoLSTRRunFERecord(Problem,Problem.FE,totalFElower,gen,upperReached);
...
function [UmaxFEs,LmaxFEs,UmaxImprFEs,LmaxImprFEs] = OriginalBLCMAESBudgets(Problem)
    if Problem.D == 20
        UmaxFEs = 5000;
        UmaxImprFEs = 750;
        LmaxFEs = 500;
