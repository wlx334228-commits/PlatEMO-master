...
% Initialize one CMA-ES state for a bounded real search space.
    ...
    if nargin < 4 || isempty(lambda)
        lambda = 4 + floor(3*log(max(D,1)));
    end
    lambda = max(4,lambda);
    mu = max(1,floor(lambda/2));
    ...
    CMA.ds = 1 + 2*max(0,sqrt((mueff-1)/(D+1))-1) + CMA.cs;
    ...
    CMA.range = range;
    CMA.xmean = xmean;
    CMA.sigma = 0.3 * median(range);
    CMA.minSigma = max(1e-10 * median(range),1e-12);
    CMA.maxSigma = max(10 * median(range),CMA.minSigma);
    CMA.ps = zeros(1,D);
    CMA.pc = zeros(1,D);
    CMA.B = eye(D);
    CMA.eigD = ones(1,D);
