...
%--------------------------------------------------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            ...
            %Iterate
            while abs(min(ul_fitness)) >= 1e-6 %Algorithm.NotTerminated(Population)
                %Selection
                ...
                best_ll_obj = llFunctionValue(best_idx,:);

                fprintf('GEN = %d | best fitness = %.6e | upper best obj = %.6e | lower best obj = %.6e\n', ...
                    GEN, min_fit, best_ul_obj, best_ll_obj);

                fprintf('upper best xu = [');
                fprintf(' %.6f', best_xu);
                fprintf(' ]\n');

                fprintf('lower best xl = [');
                fprintf(' %.6f', best_xl);
                fprintf(' ]\n');

                fprintf('upper FE = %.0f | lower FE = %.0f\n', ...
                    ulFunctionEvaluation, llFunctionEvaluation);
                fprintf('\n');
