...
%--------------------------------------------------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            ...
            %新加代码
            ulFunctionEvaluation = ulFunctionEvaluation+size(ulPopDec,1);
            ...
            %新加代码
            FitnessNow = CalFitness(Problem.C,Population);
            
            %% Optimization
            while min(FitnessNow) >= 1e-5%Algorithm.NotTerminated(Population)
                gen = gen+1;
                ...
                %新加代码
                ulFunctionEvaluation = ulFunctionEvaluation + size(ulOffDec,1);
                ...
                %新加代码
                ulFunctionEvaluation = ulFunctionEvaluation + size(eliteIndivLS,1);
                ...
                best_xu = PopDec(best_idx, 1:Problem.DU);
                best_xl = PopDec(best_idx, Problem.DU+1:end);

                best_ul_obj = PopObj(best_idx, 1);
                best_ll_obj = PopObj(best_idx, 2);

                fprintf('GEN = %d | best fitness = %.6e | best ul_obj = %.6e | best ll_obj = %.6e\n', ...
                    gen, best_fit, best_ul_obj, best_ll_obj);
                ...
                fprintf('上层函数评价次数为：%.0f\n\n', ulFunctionEvaluation);
            end
        end
    end
end
